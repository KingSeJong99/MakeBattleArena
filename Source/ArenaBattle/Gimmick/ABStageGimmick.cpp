...
void AABStageGimmick::OnGateTriggerBeginOverlap(
	...
	const FHitResult& SweepResult)
{
	/** 부딪힌 문(Gate)의 방향을 확인하여 적절한 소켓 위치에 새로운 스테이지 액터 생성 */
	...
	/** StageMesh에는 이미 +x, -X, +Y, -Y 소켓이 이미 저장되어 있음. */
	...
	/** 생성할 위치에 충돌 확인을 해서 이미 스테이지가 있는지 검증 */
	...
	/** 무언가 이미 자리를 차지하고 있다면 생성 안함 */
	if (Result)
	{
		return;
	}
	
	/** 충돌이 안됐ㅇ으면(비어있으면) 스테이지 액터 생성 */
...
void AABStageGimmick::SetReady()
{
	...
	/** 플레이어가 문과 상호작용하지 않도록 문의 콜리전을 비활성화 */
...
void AABStageGimmick::SpawnRewardBoxes()
{
	...
	for (const auto& RewardBoxLocation: RewardBoxLocations)
	{
		...
		{
			/** 상자에 태그 추가. 
			 *  나중에 어느 상자와 부딪혔는지 알기 위함입니다..
			 */
			RewardBoxActor->Tags.Add(RewardBoxLocation.Key);
		
			/** 박스가 Destroy될 때 발행되ㅑ는 이벤트에 등록. */
