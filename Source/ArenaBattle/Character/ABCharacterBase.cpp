...
void AABCharacterBase::SetComboCheckTimer()
{
	...
	float ComboEffectTime
		= (ComboActionData->EffectiveFrameCount[ComboIndex] / ComboActionData->FrameRate / AttackSpeedRate);

	// 타이머 설정.
	if (ComboEffectTime > 0)
	{
		// 시간은 월드가 관리.
		GetWorld()->GetTimerManager().SetTimer(ComboTimerHandle, this, &AABCharacterBase::ComboCheck, ComboEffectTime,
		                                       false);
	}
}
...
void AABCharacterBase::ComboActionEnd(UAnimMontage* TargetMontage, bool bInterrupted)
{
	...
	/** 몽타주 재생이 종료되면 캐릭터 이동을 다시 원상복구합니다. */
